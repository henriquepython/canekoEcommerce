...
public class SupplierRepository : GenericMongoRepository<Supplier>, ISupplierRepository
{
    private readonly IConfiguration _configuration;
    protected override string COLLECTION_NAME => CollectionNames.SUPPLIER;
    ...
    public async Task<Supplier> Create(Supplier entity)
        ...
        public async Task Delete(string id)
        {
            try
            {
                ...
                var filter = Builders<Supplier>.Filter.Eq(x => x.Id, id);
        ...
        public async Task<(IEnumerable<Supplier> products, float totalItems)> Filter(SupplierInputFilterViewModel filter)
        {
            try
            {
                if (filter == null)
        ...
        public async Task<Supplier> FindOne(string id)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id))
        ...
        public async Task<Supplier> Update(string id, Supplier entity)
        {
            try
            {
                if (entity == null)
                ...
                var filter = Builders<Supplier>.Filter.And(
                    ...
                    Builders<Supplier>.Filter.Eq(x => x.Deleted, false)
                );

                var options = new FindOneAndReplaceOptions<Supplier, Supplier>
                {
