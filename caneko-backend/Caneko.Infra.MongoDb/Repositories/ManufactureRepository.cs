...
public class ManufacturerRepository : GenericMongoRepository<Manufacturer>, IManufactureRepository
{
    private readonly IConfiguration _configuration;
    protected override string COLLECTION_NAME => CollectionNames.MANUFACTURE;
    ...
    public async Task<Manufacturer> Create(Manufacturer entity)
        {
            try
            {
                if (entity == null) 
        ...
        public async Task Delete(string id)
        {
            try
            {
                ...
                var filter = Builders<Manufacturer>.Filter.Eq(x => x.Id, id);
        ...
        public async Task<(IEnumerable<Manufacturer> products, float totalItems)> Filter(ManufacturerInputFilterViewModel filter)
        {
            try
            {
                if (filter == null)
                ...
                var filters = new List<FilterDefinition<Manufacturer>>();
        ...
        public async Task<Manufacturer> FindOne(string id)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id))
        ...
        public async Task<Manufacturer> Update(string id, Manufacturer entity)
        {
            try
            {
                if (entity == null)
                ...
                var filter = Builders<Manufacturer>.Filter.And(
                    Builders<Manufacturer>.Filter.Eq(x => x.Id, id),
                    Builders<Manufacturer>.Filter.Eq(x => x.Deleted, false)
                );

                var options = new FindOneAndReplaceOptions<Manufacturer, Manufacturer>
                {
